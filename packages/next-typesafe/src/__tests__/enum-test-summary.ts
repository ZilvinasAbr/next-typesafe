...
interface BasicEnumPage { params: { category: string } }
const test1 = createPage<BasicEnumPage>()
  .params(z.object({
    category: z.enum(['tech', 'design', 'business'])  // ✅ Works!
  }));

// ✅ Test 2: Multiple enums  
interface MultiEnumPage { params: { type: string; status: string } }
const test2 = createPage<MultiEnumPage>()
  .params(z.object({
...
interface OptionalEnumPage { params: { sort?: string } }
const test3 = createPage<OptionalEnumPage>()
  .params(z.object({
    sort: z.enum(['name', 'date']).optional()  // ✅ Works!
  }));

// ✅ Test 4: Enum with defaults
interface DefaultEnumPage { params: { format?: string } }
const test4 = createPage<DefaultEnumPage>()
  .params(z.object({
    format: z.enum(['json', 'xml']).default('json')  // ✅ Works!
  }));

// ✅ Test 5: Numeric string enums (common for versions, IDs)
interface NumericEnumPage { params: { version: string } }
const test5 = createPage<NumericEnumPage>()
  .params(z.object({
    version: z.enum(['1', '2', '3'])  // ✅ Works!
  }));

// ✅ Test 6: Mixed enums with other types
interface MixedPage { params: { id: string; category: string } }
const test6 = createPage<MixedPage>()
  .params(z.object({
    id: z.string(),                         // ✅ Works!
...
interface CountryPage { params: { country: string } }
const COUNTRIES = ['us', 'uk', 'ca', 'au', 'de', 'fr'] as const;
const test7 = createPage<CountryPage>()
  .params(z.object({
...
console.log("💡 Perfect for: categories, statuses, versions, country codes, etc.");
